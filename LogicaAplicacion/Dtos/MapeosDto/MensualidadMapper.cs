...
namespace LogicaAplicacion.Dtos.MapeosDto
{
	public class MensualidadMapper
	{
		public static Mensualidad FromDto(MensualidadDto mDto)
		{
			...
			if (mDto.suscripcion != null) mensualidad = new Mensualidad(mDto.id, mDto.suscripcionId, SuscripcionMapper.FromDto(mDto.suscripcion), mDto.periodoDesde, mDto.periodoHasta,mDto.estado);
			else mensualidad = new Mensualidad(mDto.id, mDto.suscripcionId, null, mDto.periodoDesde, mDto.periodoHasta, mDto.estado);
			return mensualidad;
		}
		...
		{
			MensualidadDto mensualidadDto = new MensualidadDto(mensualidad.Id, mensualidad.SubscriptionId, SuscripcionMapper.ToDto(mensualidad.Subscription) ?? null, mensualidad.PeriodoDesde, mensualidad.PeriodoHasta,mensualidad.Estado);
